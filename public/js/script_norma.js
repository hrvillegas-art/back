...
$(document).ready(function(){
    ...
    $('#btn-numeral').click(function(event){
        ...
        if (registros.length > 0 && registros.length < 2)
        {
            ...
            window.location.href="/numeral/"+id;
        }
    });
    ...
    $('#btn-agregar').click(function(){
        $('#form_norma')[0].reset();
        $("#titlemodal").html('Crear norma');
        $('#modal_norma').modal('open');
        metodo_save = 'POST';
        $('#id').val(0);
    });
    ...
    $('#form_norma').validate({ // initialize the plugin
        rules: {
                    id:{
                       required: true,
                        number: true,
        ...
        $.ajax({
            ...
            success: function(outserver, textStatus, xhr){
                ...
                if (xhr.status == 202) // Operacion exitosa
                {
                    ...
                    if (metodo_save == "POST")
                    {
                        mensajeConfirmacionModal('Norma registrada','¿Desea ingresar los numerales de la norma?', function(){
                            window.location.href='/numeral/'+outserver.obj.id;
                        },
                        function(){
    ...
    function listar()
    {
        ...
        $.get('/api/norma',{find:find, page:page, numero_items:numero_items} ,function(data){
            $('#body_data').html('');
    ...
    function obternerRecurso()
    {
        ...
        if (registros.length > 0 && registros.length < 2)
        {
            ...
            $.get('/api/norma/'+id,function(data){
                $('#id').val(data.id);
