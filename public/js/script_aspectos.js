...
$(document).ready(function(){
    ...
    $('#modal_aspectos').modal();

    $('#btn_modal_cancelar').click(function(){
        $('#modal_aspectos').modal('close');
    });
    ...
    $('#btn-agregar').click(function(){
        $('#form_aspectos')[0].reset();
        $("#titlemodal").html('Crear aspectos');
        $('#modal_aspectos').modal('open');
        metodo_save = 'POST';
        $('#id').val(0);
    });
    ...
    $('#btn-editar').click(function(){
        metodo_save = 'PUT';
        $("#titlemodal").html('Editar aspectos');
        $('#form_aspectos')[0].reset();
        obternerRecurso();
    });
    ...
    $('#btn_modal_guardar').click(function(){
        send = true;
        $('#form_aspectos').submit();
    });
    $('#form_aspectos').validate({ // initialize the plugin
        rules: {
                    id:{
                       required: true,
                        number: true,
        ...
        messages: {
                    ...
                    caracteristicas_id:{
                        required: "El campo Característica es obligatorio",
                        number: "El campo Característica deber numérico",
                        min: "El campo Característica deber de mínimo 1"
                        },
                    codigo:{
                        required: "El campo Código del aspecto es obligatorio",
                        maxlength: "El campo Código del aspecto fuera de rango"
                        },
                    nombre:{
                        required: "El campo Nombre del aspecto es obligatorio",
                        maxlength: "El campo Nombre del aspecto fuera de rango"
                        },
                    descripcion:{
                        required: "El campo Descripción del aspecto es obligatorio",
                        maxlength: "El campo Descripción del aspecto fuera de rango"
                        }    
        },
        errorElement : 'div',
    ...
    function listar()
    {
        ...
        $.get('/api/aspectos',{find:find, page:page, numero_items:numero_items} ,function(data){
            $('#body_data').html('');
            ...
            $.each(array_data, function (key, val){
                ...
                <tr >\
                    ...
                    <td>'+val.caracteristicas_nombre+'</td>\
                    <td>'+val.codigo+'</td>\
                    <td>'+val.nombre+'</td>\
    ...
    function obternerRecurso()
    {
        ...
        if (registros.length > 0 && registros.length < 2)
        {
            ...
            $.get('/api/aspectos/'+id,function(data){
                $('#id').val(data.id);
                $('#caracteristicas_id').val(data.caracteristicas_id);
