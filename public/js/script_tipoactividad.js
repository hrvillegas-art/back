...
$(document).ready(function(){
    ...
    $('#modal_tipoactividad').modal();

    $('#btn_modal_cancelar').click(function(){
        $('#modal_tipoactividad').modal('close');
    });
    ...
    $('#btn-agregar').click(function(){
        $('#form_tipoactividad')[0].reset();
        $("#titlemodal").html('Crear tipo de actividad');
        $('#modal_tipoactividad').modal('open');
        metodo_save = 'POST';
        $('#id').val(0);
    });
    ...
    $('#btn-editar').click(function(){
        metodo_save = 'PUT';
        $("#titlemodal").html('Editar tipo de actividad');
        $('#form_tipoactividad')[0].reset();
        obternerRecurso();
    });
    ...
    $('#btn_modal_guardar').click(function(){
        send = true;
        $('#form_tipoactividad').submit();
    });
    $('#form_tipoactividad').validate({ // initialize the plugin
        rules: {
                    id:{
                       required: true,
                        number: true,
        ...
        messages: {
                    ...
                    nombre:{
                        required: "El campo Nombre del tipo es obligatorio",
                        maxlength: "El campo Nombre del tipo fuera de rango"
                        },
                    descripcion:{
                        required: "El campo Descripción del tipo es obligatorio",
                        maxlength: "El campo Descripción del tipo fuera de rango"
                        }    
        },
        errorElement : 'div',
    ...
    {
        $.ajax({
            url: '/api/tipoactividad',  
            type: metodo_save,
            headers:{'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            data: $('#form_tipoactividad').serialize(),
            beforeSend: function(){
    ...
    function listar()
    {
        ...
        $.get('/api/tipoactividad',{find:find, page:page, numero_items:numero_items} ,function(data){
            $('#body_data').html('');
    ...
    {
        $.ajax({
            url: '/api/tipoactividad/'+ids+'/activo',  
            type: 'PUT',
    ...
    function obternerRecurso()
    {
        ...
        if (registros.length > 0 && registros.length < 2)
        {
            ...
            $.get('/api/tipoactividad/'+id,function(data){
                $('#id').val(data.id);
