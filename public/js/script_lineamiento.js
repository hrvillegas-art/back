...
$(document).ready(function(){
    ...
    $('#modal_lineamiento').modal();

    $('#btn_modal_cancelar').click(function(){
        $('#modal_lineamiento').modal('close');
    });
    ...
    $('#btn-agregar').click(function(){
        $('#form_lineamiento')[0].reset();
        $("#titlemodal").html('Crear lineamiento');
        $('#modal_lineamiento').modal('open');
        metodo_save = 'POST';
        $('#id').val(0);
    });
    ...
    $('#btn-editar').click(function(){
        metodo_save = 'PUT';
        $("#titlemodal").html('Editar lineamiento');
        $('#form_lineamiento')[0].reset();
        obternerRecurso();
    });
    ...
    $('#btn_modal_guardar').click(function(){
        send = true;
        $('#form_lineamiento').submit();
    });

    $('#form_lineamiento').validate({ // initialize the plugin
        rules: {
                    id:{
                       required: true,
                        number: true,
        ...
        messages: {
                    ...
                    tipolineamiento_id:{
                        required: "El campo tipo de lineamiento es obligatorio",
                        number: "El campo tipo de lineamiento deber numérico",
                        min: "El campo tipo de lineamiento deber de mínimo 1"
                        },
                    anio:{
                        required: "El campo año es obligatorio",
                        number: "El campo año deber numérico",
                        min: "El campo año deber tener minimo 4 caracteres",
                        maxlength: "El campo debe tener 4 caracteres",
                        },
                    nombre:{
                        required: "El campo Nombre del lineamiento es obligatorio",
                        maxlength: "El campo Nombre del lineamiento fuera de rango"
                        },
                    descripcion:{
                        required: "El campo Descripción del lineamiento es obligatorio",
                        maxlength: "El campo Descripción del lineamiento fuera de rango"
                        }    
        },
        errorElement : 'div',
    ...
    function guardar()
    {
        $.ajax({
            ...
            data: $('#form_lineamiento').serialize(),
            beforeSend: function(){
    ...
    function listar()
    {
        ...
        $.get('/api/lineamiento',{find:find, page:page, numero_items:numero_items} ,function(data){
            $('#body_data').html('');
            ...
            $.each(array_data, function (key, val){
                ...
                <tr >\
                    ...
                    <td>'+val.tipolineamiento_nombre+'</td>\
                    <td>'+val.anio+'</td>\
                    <td>'+val.nombre+'</td>\
    ...
    {
        $.ajax({
            url: '/api/lineamiento/'+ids+'/activo',  
            type: 'PUT',
    ...
    function obternerRecurso()
    {
        ...
        if (registros.length > 0 && registros.length < 2)
        {
            ...
            $.get('/api/lineamiento/'+id,function(data){
                $('#id').val(data.id);
                $('#tipolineamiento_id').val(data.tipolineamiento_id);
                $('#anio').val(data.anio);
