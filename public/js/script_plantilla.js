...
$(document).ready(function(){
    ...
    $('#modal_plantilla').modal();

    $('#btn_modal_cancelar').click(function(){
        $('#modal_plantilla').modal('close');
    });
    ...
    $('#btn-agregar').click(function(){
        $('#form_plantilla')[0].reset();
        $("#titlemodal").html('Crear plantilla');
        $('#modal_plantilla').modal('open');
        metodo_save = 'POST';
        $('#id').val(0);
    });
    ...
    $('#tipoextension').change(function(event){

        $( "#archivo_url" ).rules( "remove", "extension" );
        ...
        switch ($(this).val()) {
            case 'docx':
                $( "#archivo_url" ).rules( "add", {
                    extension:'docx',
                    messages: {
                        extension: "Extensión no permitida, solo se puede adjuntar archivos en formato word (docx)",
                    }
                  });
                break;
            case 'xlsx':
                $( "#archivo_url" ).rules( "add", {
                    extension:'xlsx',
                    messages: {
                        extension: "Extensión no permitida, solo se puede adjuntar archivos en formato excel (xlsx)",
                    }
                  });
                break;
                case 'pdf':
                    $( "#archivo_url" ).rules( "add", {
                        extension:'pdf',
                        messages: {
                            extension: "Extensión no permitida, solo se puede adjuntar archivos en formato pdf (pdf)",
                        }
                      });
                    break;
            default:
    ...
    $('#form_plantilla').validate({ // initialize the plugin
        rules: {
                    id:{
                       required: true,
                        number: true,
        ...
        messages: {
                    ...
                    tipoextension:{
                        required: "El campo tipoextension es obligatorio",
                        maxlength: "El campo tipoextension fuera de rango"
                        },
                    archivo_url:{
                        required: "El campo archivo_url es obligatorio",
                        maxlength: "El campo archivo_url fuera de rango",
                        extension: "Extensión no permitida, solo se puede adjuntar archivos en formato word"
                        },
                    descripcion:{
    ...
    function guardar()
    {
        var formData = new FormData(document.getElementById("form_plantilla"));
    ...
    function listar()
    {
        ...
        $.get('/api/plantilla',{find:find, page:page, numero_items:numero_items} ,function(data){
            $('#body_data').html('');
            ...
            $.each(array_data, function (key, val){
                ...
                <tr >\
                    ...
                    <td>'+val.nombre+'</td>\
                    <td>'+val.tipoextension+'</td>\
                    <td><a target="_blank" href="'+BASE_PUBLIC_PATH+'/Evidenciaplantilla/'+val.archivo_url+'">Ver evidencia</a></td>\
    ...
    function obternerRecurso()
    {
        ...
        if (registros.length > 0 && registros.length < 2)
        {
            ...
            $.get('/api/plantilla/'+id,function(data){
                $('#id').val(data.id);
                $('#nombre').val(data.nombre);
                $('#tipoextension').val(data.tipoextension).change();
