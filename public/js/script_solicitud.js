...
$(document).ready(function(){
    ...
    $('#btn-editar').click(function(){
        metodo_save = 'POST';
        $("#title_div_documento").html('Editar solicitud');
        $('#form_documento')[0].reset();
        limpiarArrrays();
        obternerRecurso();
    });
    ...
    $('#btn-eliminar').click(function(){
        ...
        mensajeConfirmacionModal('Eliminar recurso', 'Solo se eliminarán las solicitudes en estado '+estadoSolicitud('P')+'.<br>¿Esta seguro que desea eliminar los recursos?', function(){
            eliminar(array_ids);
        });
    ...
    function listar()
    {
        ...
        $.get('/api/solicitud',{find:find, proceso_id:find_proceso_id, page:page, numero_items:numero_items} ,function(data){
            $('#body_data').html('');
    ...
    function obternerRecurso()
    {
        ...
        if (registros.length > 0 && registros.length < 2)
        {
            ...
            $.get('/api/solicitud/'+id,function(data){
                if (data.estado == 'P' || data.estado == 'R')
                {
                    $('#div_documento').removeClass('display-none');
                    $('#div_listardocumento').addClass('display-none');
                ...
                {
                    mensajeErrorModal('Operación invalida', 'La solicitud solo es editable cuando se encuentra en estado '+estadoSolicitud('P')+' o '+estadoSolicitud('R'));
                }
