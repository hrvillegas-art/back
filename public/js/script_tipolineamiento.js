...
$(document).ready(function(){
    ...
    $('#modal_tipoprograma').modal();

    $('#btn_modal_cancelar').click(function(){
        $('#modal_tipoprograma').modal('close');
    });
    ...
    $('#btn-agregar').click(function(){
        $('#form_tipoprograma')[0].reset();
        $("#titlemodal").html('Crear tipo de lineamiento');
        $('#modal_tipoprograma').modal('open');
        metodo_save = 'POST';
        $('#id').val(0);
    });
    ...
    $('#btn-editar').click(function(){
        metodo_save = 'PUT';
        $("#titlemodal").html('Editar tipo de lineamiento');
        $('#form_tipoprograma')[0].reset();
        obternerRecurso();
    });
    ...
    $('#btn_modal_guardar').click(function(){
        send = true;
        $('#form_tipoprograma').submit();
    });
    $('#form_tipoprograma').validate({ // initialize the plugin
        rules: {
                    id:{
                       required: true,
                        number: true,
        ...
        messages: {
                    ...
                    nombre:{
                        required: "El campo Nombre del es obligatorio",
                        maxlength: "El campo Nombre del fuera de rango"
                        },
                    descripcion:{
                        required: "El campo Descripción del es obligatorio",
                        maxlength: "El campo Descripción del fuera de rango"
                        }    
        },
        errorElement : 'div',
    ...
    {
        $.ajax({
            url: '/api/tipolineamiento',  
            type: metodo_save,
            headers:{'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            data: $('#form_tipoprograma').serialize(),
            beforeSend: function(){
    ...
    function listar()
    {
        ...
        $.get('/api/tipolineamiento',{find:find, page:page, numero_items:numero_items} ,function(data){
            $('#body_data').html('');
    ...
    {
        $.ajax({
            url: '/api/tipolineamiento/'+ids+'/activo',  
            type: 'PUT',
    ...
    function obternerRecurso()
    {
        ...
        if (registros.length > 0 && registros.length < 2)
        {
            ...
            $.get('/api/tipolineamiento/'+id,function(data){
                $('#id').val(data.id);
