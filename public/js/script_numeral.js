...
$(document).ready(function(){
    ...
    $('#btn-agregar').click(function(){
        $('#form_numeral')[0].reset();
        $("#titlemodal").html('Crear numeral');
        $('#modal_numeral').modal('open');
        metodo_save = 'POST';
        $('#id').val(0);
    });
    ...
    $('#btn-editar').click(function(){
        metodo_save = 'PUT';
        $("#titlemodal").html('Editar numeral');
        $('#form_numeral')[0].reset();
        obternerRecurso();
    });
    ...
    $('#form_numeral').validate({ // initialize the plugin
        rules: {
                    id:{
                       required: true,
                        number: true,
        ...
        messages: {
                    ...
                    norma_id:{
                        required: "El campo Norma relacionada es obligatorio",
                        number: "El campo Norma relacionada deber numérico",
                        min: "El campo Norma relacionada deber de mínimo 1"
                        },
                    numeral:{
                        required: "El campo Numeral es obligatorio",
                        maxlength: "El campo Numeral fuera de rango"
                        },
                    nombre:{
    ...
    function listar()
    {
        ...
        page = last_page,
        find =  $('#find').val(),
        norma_id = $("#norma_id").val(),
        numero_columnas = $(".table").children("thead").children('tr').children('th').length;

        $('#body_data').html(loadingPlain(numero_columnas));

        $.get('/api/numeral',{find:find, norma_id:norma_id, page:page, numero_items:numero_items} ,function(data){
            $('#body_data').html('');
    ...
    function obternerRecurso()
    {
        ...
        if (registros.length > 0 && registros.length < 2)
        {
            ...
            $.get('/api/numeral/'+id,function(data){
                $('#id').val(data.id);
                $('#norma_id').val(data.norma_id);
                $('#numeral').val(data.numeral);
