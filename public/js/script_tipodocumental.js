...
$(document).ready(function(){
    ...
    $('#modal_tipodocumental').modal();

    $('#btn_modal_cancelar').click(function(){
        $('#modal_tipodocumental').modal('close');
    });
    ...
    $('#btn-agregar').click(function(){
        $('#form_tipodocumental')[0].reset();
        $("#titlemodal").html('Crear tipo documental');
        $('#modal_tipodocumental').modal('open');
    ...
    $('#btn-editar').click(function(){
        metodo_save = 'PUT';
        $("#titlemodal").html('Editar tipo documental');
        $('#form_tipodocumental')[0].reset();
        $('input[type="checkbox"]').attr('checked', false);
        obternerRecurso();
    });
    ...
    $('#btn_modal_guardar').click(function(){
        send = true;
        $('#form_tipodocumental').submit();
    });

    $('#form_tipodocumental').validate({ // initialize the plugin
        rules: {
                    id:{
                       required: true,
                        number: true,
        ...
        messages: {
                    ...
                    'plantilla_id[]':{
                        required: "El campo Plantilla a utilizar es obligatorio",
                        number: "El campo Plantilla a utilizar deber numérico",
                        },
                    nombre:{
                    ...
                    prefijo:{
                        ...
                        maxlength: "El campo Prefijo fuera de rango"
                        }   
        },
        errorElement : 'div',
    ...
    {
        $.ajax({
            url: '/api/tipodocumental',  
            type: metodo_save,
            headers:{'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            data: $('#form_tipodocumental').serialize(),
            beforeSend: function(){
    ...
    function listar()
    {
        ...
        $.get('/api/tipodocumental',{find:find, page:page, numero_items:numero_items} ,function(data){
            $('#body_data').html('');
            ...
            $.each(array_data, function (key, val){
                ...
                val.array_plantilla.forEach(element => {
                    array_plantilla.push(element.obj_plantilla.nombre);
                });
                ...
                {
                    array_plantilla.push('Por formulario definido en la aplicación');
                }
    ...
    {
        $.ajax({
            url: '/api/tipodocumental/'+ids+'/activo',  
            type: 'PUT',
    ...
    function obternerRecurso()
    {
        ...
        if (registros.length > 0 && registros.length < 2)
        {
            ...
            $.get('/api/tipodocumental/'+id,function(data){
                $('#id').val(data.id);
                $('#nombre').val(data.nombre);
                $('#prefijo').val(data.prefijo);

                data.array_plantilla.forEach(element => {
                    $('#plantilla_id_'+element.plantilla_id).attr('checked', true);
                });
